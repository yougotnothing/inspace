...
class KeycloakClient:
  def __init__(self):
    try:
      self.keycloak_admin = KeycloakAdmin(connection=KeycloakOpenIDConnection(
      ...
      self.keycloak_openid = KeycloakOpenID(server_url=os.getenv('KC_SERVER_URL'),
                                          client_id=os.getenv('KC_CLIENT_ID'),
                                          realm_name=os.getenv('KC_REALM'),
                                          client_secret_key=os.getenv('KC_SECRET'))
  ...
  def create_realm_if_not_exists(self):
    try:
      realm = self.keycloak_admin.get_realm(realm_name=os.getenv('KC_REALM'))
      ...
      if realm:
        ...
        self.keycloak_admin.create_realm(payload=realm_config)
